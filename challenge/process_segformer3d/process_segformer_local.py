...
class Uls23(SegmentationAlgorithm):
    ...
    def load_model(self, model_dir):
        ...
        config_path = Path(f"./{model_dir}/config.yaml")
        ...
        model_bin_path = Path(f"./{model_dir}pytorch_model.bin")
    ...
    def load_data(self):
        ...
        for input_file in input_dir.glob("*.mha"):
            ...
            print(f"Loaded image {input_file.name} with shape {image_data.shape} and spacing {self.image_metadata.GetSpacing()}")
            for i in range(int(image_data.shape[0] / self.z_size)):
                ...
                # Save the metadata to reorient back from RAS
                metadata = {
                "direction": self.image_metadata.GetDirection(),
                "origin": self.image_metadata.GetOrigin(),
                "spacing": self.image_metadata.GetSpacing()
                }
                with open(f"./tmp/voi_{i}_metadata.json", "w") as f:
                    # Convert tuples to lists for JSON serialization
                    json.dump({k: list(v) for k, v in metadata.items()}, f)
